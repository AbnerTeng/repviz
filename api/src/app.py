...
from utils import list_child_modules
    ...
    if st.session_state.training_monitor is not None:
        ...
        if "CKA" in analysis_types:
            ...
            for i in range(len(activations[selected_layer])):
                for layer_name in layer_names:
                    ...
                    cka_over_time[layer_name].append(
                        cka.linear_cka(
                            act1.view(act1.shape[0], -1), act2.view(act2.shape[0], -1)
                        )
                    )
        ...
        with col2:
            ...
            if input_data is not None:
                fig, ax = plt.subplots()
                sns.scatterplot(
                    x=input_data[-1].flatten().cpu().numpy(),
                    y=data[-1].flatten().cpu().numpy(),
                    ax=ax,
                )
        ...
        with col1:
            if "PCA" in analysis_types:
                st.subheader("PCA")
                pca_data = dim_reduction.decomposition(
                    activations[selected_layer][-1].view(
                        activations[selected_layer][-1].shape[0], -1
                    ),
                    2,
                    "PCA",
                )
        ...
        with col2:
            if "UMAP" in analysis_types:
                st.subheader("UMAP")
                umap_data = dim_reduction.plot_umap(
                    activations[selected_layer][-1].view(
                        activations[selected_layer][-1].shape[0], -1
                    ),
                    selected_layer,
